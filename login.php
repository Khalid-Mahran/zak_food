...
$login_error=''; $reg_error=''; $reg_success=''; $active_tab='login';
...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['action'])&&$_POST['action']==='login'){
    ...
    if($result&&mysqli_num_rows($result)===1){
        ...
        if(!password_verify($password,$user['password'])){$login_error='Wrong password!';}
        elseif($user['role']==='vendor'&&intval($user['is_approved'])!==1){$login_error='Account pending admin approval.';}
        else{$_SESSION['user_id']=$user['id'];$_SESSION['name']=$user['name'];$_SESSION['role']=$user['role'];redirectByRole($user['role']);}
...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['action'])&&$_POST['action']==='register'){
    ...
    if($role==='admin'){$reg_error='Admin registration not allowed.';}
    else{
        $check=mysqli_query($conn,"SELECT id FROM users WHERE email='$email'");
        if(mysqli_num_rows($check)>0){$reg_error='Email already exists.';}
        else{
...
.burger-stage{position:relative;width:240px;height:160px;display:flex;align-items:flex-end;justify-content:center;margin-bottom:-2px;z-index:3;}

.bubble{position:absolute;top:-8px;left:50%;transform:translateX(-50%) scale(0);background:#fff0f0;border:2.5px solid #fca5a5;border-radius:18px;padding:9px 16px;font-size:13px;font-weight:700;color:#dc2626;white-space:nowrap;transform-origin:bottom center;transition:transform 0.35s cubic-bezier(.34,1.56,.64,1);pointer-events:none;z-index:10;box-shadow:0 4px 16px rgba(220,38,38,0.15);}
...
@keyframes bshake{0%,100%{transform:translateX(0)}12%{transform:translateX(-12px)}25%{transform:translateX(12px)}37%{transform:translateX(-9px)}50%{transform:translateX(9px)}62%{transform:translateX(-5px)}75%{transform:translateX(5px)}87%{transform:translateX(-2px)}}
.shaking{animation:bshake 0.65s ease-in-out;}

.arm-l{transform-origin:68px 138px;transform:rotate(55deg);transition:transform 0.5s cubic-bezier(.34,1.56,.64,1);transform-box:fill-box;}
.arm-r{transform-origin:132px 138px;transform:rotate(-55deg);transition:transform 0.5s cubic-bezier(.34,1.56,.64,1);transform-box:fill-box;}
.covering .arm-l{transform:rotate(-115deg);}
.covering .arm-r{transform:rotate(115deg);}
.peeking .arm-l{transform:rotate(-80deg);}
.peeking .arm-r{transform:rotate(92deg);}
.blush{transition:opacity 0.35s;}

.auth-card{background:white;border-radius:24px;width:100%;max-width:460px;overflow:hidden;box-shadow:0 28px 80px rgba(0,0,0,0.35);position:relative;z-index:2;}
.auth-header{background:linear-gradient(135deg,#f97316,#c2410c);padding:22px 40px 20px;text-align:center;}
...
.pw-eye{position:absolute;right:12px;top:50%;transform:translateY(-50%);border:none;background:none;cursor:pointer;font-size:18px;padding:4px;line-height:1;color:#64748b;border-radius:6px;transition:color 0.2s;display:flex;align-items:center;justify-content:center;width:32px;height:32px;}
.pw-eye:hover{color:#0f172a;background:#f1f5f9;}
</style>

<div class="auth-page">
  <div class="burger-stage">
    <div class="bubble" id="bub">Wrong email or password!</div>
    <svg id="bsvg" viewBox="0 0 200 200" width="210" height="155" style="overflow:visible;display:block;">

      <!-- Arms (drawn first so burger renders on top) -->
      <g class="arm-l" id="armL">
        <rect x="4" y="129" width="66" height="19" rx="9" fill="#D4852A"/>
        <circle cx="13" cy="138" r="13" fill="#E8A855"/>
        <ellipse cx="6" cy="131" rx="3.5" ry="2.5" fill="#D4852A"/>
        <ellipse cx="5" cy="138" rx="3.5" ry="2.5" fill="#D4852A"/>
        <ellipse cx="6" cy="145" rx="3.5" ry="2.5" fill="#D4852A"/>
      </g>
      <g class="arm-r" id="armR">
        <rect x="130" y="129" width="66" height="19" rx="9" fill="#D4852A"/>
        <circle cx="187" cy="138" r="13" fill="#E8A855"/>
        <ellipse cx="194" cy="131" rx="3.5" ry="2.5" fill="#D4852A"/>
        <ellipse cx="195" cy="138" rx="3.5" ry="2.5" fill="#D4852A"/>
        <ellipse cx="194" cy="145" rx="3.5" ry="2.5" fill="#D4852A"/>
      </g>

      <!-- Bottom bun -->
      <ellipse cx="100" cy="172" rx="68" ry="21" fill="#E8A855"/>
      <ellipse cx="100" cy="167" rx="63" ry="9" fill="#D4852A" opacity="0.2"/>
      <!-- Patty -->
      <rect x="30" y="144" width="140" height="24" rx="8" fill="#6B3320"/>
      <rect x="30" y="144" width="140" height="7" rx="3" fill="#7D4535" opacity="0.35"/>
      <ellipse cx="58" cy="144" rx="6" ry="3.5" fill="#7D4535" opacity="0.4"/>
      <ellipse cx="100" cy="142" rx="6" ry="3.5" fill="#7D4535" opacity="0.4"/>
      <ellipse cx="142" cy="144" rx="6" ry="3.5" fill="#7D4535" opacity="0.4"/>
      <!-- Cheese -->
  ...
  <div class="auth-card">
    ...
    <div class="auth-body" id="body-login" style="display:<?php echo $active_tab==='login'?'block':'none';?>">
      <?php if($login_error): ?><div class="alert"><?php echo $login_error; ?></div><?php endif; ?>
      <?php if($reg_success): ?><div class="alert success"><?php echo $reg_success; ?></div><?php endif; ?>
      <form method="POST">
        ...
        <div class="pw-wrap">
          <input type="password" name="password" id="pw" placeholder="&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;" required>
          <button class="pw-eye" id="eyeBtn" type="button" title="Show password">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" id="eyeIcon"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
          </button>
        </div>
        <br>
    ...
    <div class="auth-body" id="body-reg" style="display:<?php echo $active_tab==='register'?'block':'none';?>">
      <?php if($reg_error): ?><div class="alert"><?php echo $reg_error; ?></div><?php endif; ?>
      <form method="POST">
        ...
        <label>Password</label>
        <div class="pw-wrap">
          <input type="password" name="password" id="pw2" placeholder="&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;" required>
          <button class="pw-eye" type="button" onclick="var i=this.previousElementSibling;var v=i.type==='password';i.type=v?'text':'password';this.innerHTML=v?'<svg width=20 height=20 viewBox=&quot;0 0 24 24&quot; fill=none stroke=currentColor stroke-width=2 stroke-linecap=round><path d=&quot;M17.94 17.94A10.07 10.07 0 0112 20c-7 0-11-8-11-8a18.45 18.45 0 015.06-5.94&quot;/><path d=&quot;M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a18.5 18.5 0 01-2.16 3.19&quot;/><line x1=1 y1=1 x2=23 y2=23/></svg>':'<svg width=20 height=20 viewBox=&quot;0 0 24 24&quot; fill=none stroke=currentColor stroke-width=2 stroke-linecap=round><path d=&quot;M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z&quot;/><circle cx=12 cy=12 r=3/></svg>';" title="Show/hide password">
...
(function(){
  ...
  var bwL=document.getElementById('bwL'),bwR=document.getElementById('bwR');
  var bshL=document.getElementById('bshL'),bshR=document.getElementById('bshR');
  var lidL=document.getElementById('lidL'),lidR=document.getElementById('lidR');
  var state='normal',pwVis=false;
  ...
  <?php if($login_error): ?>
  setTimeout(function(){error(<?php echo json_encode($login_error);?>);},200);
  <?php endif; ?>
})();
